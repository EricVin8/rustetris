...
impl Blocks {
    pub fn rotate(&mut self, win: &Window) {
        ...
        for i in 0..self.shape.len() {
            for j in 0..self.shape.len() {
                if temparr[i][j] == '#'
                    && (win.mvinch(self.lefty + i as i32, self.leftx + j as i32) & 0x7F) == 35
                    || self.lefty + (i as i32) > self.maxy
                    || self.leftx + (j as i32) > self.maxx
                    || self.leftx + (j as i32) < self.minx
                {
                    swapsuccess = false;
                }
            }
    ...
    fn check_futurecoords(&mut self, newx: i32, newy: i32, win: &Window) -> bool {
        ...
        for i in 0..self.shape.len() {
            for j in 0..self.shape.len() {
                ...
                if self.shape[i][j] == '#'
                    && (newy + i as i32 > self.maxy
                        || newx + j as i32 > self.maxx
                        || (newx + j as i32) < self.minx)
                {
                    success = false;
                }
                if self.shape[i][j] == '#'
                    && ((win.mvinch(newy + i as i32, newx + j as i32)) & 0x7F) == 35
                {
                    ...
                    if !((j as i32 + (newx - self.leftx)) as i32 >= 0
                        && i as i32 + (newy - self.lefty) >= 0
                        && j as i32 + (newx - self.leftx) < self.shape.len() as i32
                        && i as i32 + (newy - self.lefty) < self.shape.len() as i32
                        && self.shape[(i as i32 + (newy - self.lefty)) as usize]
                            [(j as i32 + (newx - self.leftx)) as usize]
                            == '#')
                    {
                        success = false;
                    }
