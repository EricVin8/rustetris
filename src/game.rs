...
pub fn genblockonscreen(
    ...
    block::Blocks {
        leftx: x,
        lefty: spawny,
        minx: x - (boardwidth as i32 / 2) + 1,
        maxx: x + (boardwidth as i32 / 2),
        miny: spawny,
        maxy: y + (boardheight as i32) / 2,
...
pub fn updatescore(window: &Window, score: &i32) {
    ...
    window.mvaddstr(
        2,
        window.get_max_x() - stringscore.len() as i32 - 1,
        stringscore,
    );
    window.refresh();
}
...
pub fn clearline(block: block::Blocks, window: &Window, score: &mut i32) -> bool {
    ...
    for i in block.miny..block.maxy + 1 {
        ...
        for j in block.minx..block.maxx + 1 {
            ...
            board[(i - block.miny) as usize][(j - block.minx) as usize] =
                window.mvinch(i, j) as u64;
        }
        if fulline {
            changed = true;
        ...
        for i in block.miny..block.maxy + 1 {
            for j in block.minx..block.maxx + 1 {
                window.mvaddch(
                    i,
                    j,
                    board[(i - block.miny) as usize][(j - block.minx) as usize] as u32,
                );
            }
        }
    }
